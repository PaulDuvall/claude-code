...
##################################
# Setting up API access (conditional based on authentication method)
# - Modern Claude Code uses web-based authentication by default
# - API keys are only needed for CI/CD, corporate setups, or headless environments
# - When API key is provided, we create a helper script for secure access
# idea via https://www.reddit.com/r/ClaudeAI/comments/1jwvssa/claude_code_with_api_key/
##################################
# Setup API helper (only if using API key)
##################################
...
#
# Configure API key approval if using API key authentication
##################################
if [[ "$USE_API_KEY" == "true" ]]; then
    ANTHROPIC_API_KEY_LAST_20_CHARS=${ANTHROPIC_API_KEY: -20}
    # We write the global config to ~/.claude.json with API key approval
...
# Configure API helper in settings (only if using API key)
if [[ "$USE_API_KEY" == "true" ]]; then
    if [[ "$DRY_RUN" == "true" ]]; then
